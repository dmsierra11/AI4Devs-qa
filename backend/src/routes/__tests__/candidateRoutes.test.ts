...
describe('POST /candidates', () => {
    ...
    it('should responde with 400 if email already exists', async () => {
        ...
        const res = await request(server).post('/candidates').send(candidateData);
...
describe('PUT /candidates/:id', () => {
    it('should update candidate stage and return 200 status', async () => {
        ...
        const candidateId = 3;
        const res = await request(server).put(`/candidates/${candidateId}`).send(updateData);
...
describe('GET /candidates/:id', () => {
    it('should retrieve candidate data and return 200 status', async () => {
        const candidateId = 1;
        const res = await request(server).get(`/candidates/${candidateId}`);
        expect(res.status).toBe(200);
